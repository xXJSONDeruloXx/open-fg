...
# Best-effort: trigger both documented paths.
steam steam://uninstall/${APPID} >/tmp/steam-uninstall-${APPID}.log 2>&1 &
steam steam://open/console >/tmp/steam-open-console-${APPID}.log 2>&1 &
sleep 3

for wid in $(xdotool search --class Steam || true); do
  xdotool key --window "$wid" Return || true
  xdotool key --window "$wid" Return || true
  xdotool type --window "$wid" --delay 40 "app_uninstall -complete ${APPID}" || true
  xdotool key --window "$wid" Return || true
done
...
if [ -f "$CONTENT_LOG" ]; then
  tail -80 "$CONTENT_LOG" | grep "AppID ${APPID}\|state changed\|finished update\|uninstall" || true
fi
exit 2
EOF
)
